...
let mongoConn = {
  ...
  }:${process.env.MONGO_DB_PORT}/?authMechanism=DEFAULT&authSource=admin`,
...
let connect = (url, options, clgDB) => {
  try {
    return new Promise((resolve) => {
      resolve(
        MongoClient.connect(url, options).then((client) => client.db(clgDB))
      );
    });
  } catch (e) {
    console.log(e);
...
export const knex = knex0(
  connectionKnex[process.env.NODE_ENV || "development"]
);
