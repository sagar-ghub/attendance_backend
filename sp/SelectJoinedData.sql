drop procedure if exists SelectJoinedData;
DELIMITER $$
/* CALL SelectJoinedData('{
					"table_name" : "rp_user",
					"data" : ["rp_user.user_id as userId", "rp_file.file_path as filePath"] ,
                    "joinString": "left join rp_file on rp_user.user_img = rp_file.file_id",
                    "cond" : [{"field" : "user_status", "opt" : "=", "value" : "inactive","type":"AND"}]
                    }'); */
create procedure SelectJoinedData(in p_table_json json)
    begin
        declare v_table_name varchar(255) CHARSET utf8 default null;
        declare v_join_string varchar(255) CHARSET utf8 default null;
        ...
        if JSON_VALID(p_table_json) then
            ...
            if (v_table_name is not null) then
                ...
                #making final SQL
                set @v_final_query = CONCAT('SELECT ', v_select_query, ' FROM ', JSON_UNQUOTE(v_table_name),' ', JSON_UNQUOTE(v_join_string) ,'',' WHERE 1', v_condition_query,';');
